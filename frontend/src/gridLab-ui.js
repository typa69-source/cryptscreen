...
// ═══════════════════════════════════════════════════════════════
//  Modal host (Step 3B.5)
// ═══════════════════════════════════════════════════════════════

/**
 * Build the backtest tab HTML for the Grid Lab modal.
 * Pure-ish: depends only on gbPrefs and the resolved default symbol.
 */
export function buildGridLabBacktestMarkup(gbPrefs, defSym) {
  const sb = (gbPrefs.symbolBounds && gbPrefs.symbolBounds[defSym]) || {};
  const opt = (sel, val) => sel === val ? ' selected' : '';
  const tf = gbPrefs.global.tf;
  const gm = gbPrefs.global.gridMode;
  return `
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:0;flex-shrink:0">
        ...
        <select id="gbTf" style="width:62px;background:var(--bg3);border:1px solid var(--border2);border-radius:4px;color:var(--text);font:inherit;font-size:10px;padding:3px 6px">
          <option value="1m"${opt('1m', tf)}>1m</option>
          <option value="5m"${opt('5m', tf)}>5m</option>
          <option value="15m"${opt('15m', tf)}>15m</option>
          <option value="1h"${opt('1h', tf)}>1h</option>
        </select>
        ...
        <select id="gbGridMode" style="width:132px;background:var(--bg3);border:1px solid var(--border2);border-radius:4px;color:var(--text);font:inherit;font-size:10px;padding:3px 6px">
          <option value="neutral"${opt('neutral', gm)}>Neutral</option>
          <option value="long"${opt('long', gm)}>Long</option>
          <option value="short"${opt('short', gm)}>Short</option>
        </select>
      </div>
...
/**
 * Build the coin-selector tab HTML.
 * Pure: takes precomputed rows and a format function.
 */
function buildGridLabSelectorMarkup(rows, fn, fk, openSymFn) {
  const header = `
      ...
      <div style="font-size:9px;color:var(--text3);line-height:1.4;margin:0 0 9px 0">
        ...
        <div style="display:grid;grid-template-columns:110px repeat(6,1fr);gap:6px;font-size:9px">
          <div style="color:var(--text3)">Символ</div><div style="color:var(--text3)">GridScore</div><div style="color:var(--text3)">Ренж24</div><div style="color:var(--text3)">NATR5m</div><div style="color:var(--text3)">Δ24</div><div style="color:var(--text3)">Объём24</div><div style="color:var(--text3)">Сделки24</div>
          ${rows.map((r) => `
            <div style="font-weight:600;color:#fff;cursor:pointer" onclick="${openSymFn}('${r.sym}')">${r.sym.replace(/USDT$/, '')}</div>
            <div>${fn(r.score, 1)}</div>
            <div>${fn(r.range24, 2)}%</div>
            <div>${fn(r.natr, 2)}%</div>
            <div class="${r.ch24 >= 0 ? 'p' : 'n'}">${r.ch24 >= 0 ? '+' : ''}${fn(r.ch24, 2)}%</div>
            <div>${fk(r.vol24)}</div>
            <div>${fk(r.trd24)}</div>
          `).join('')}
        </div>`;
...
/**
 * Cleanup any chart / poll / abort-controller resources tied to the Grid Lab body.
 * Safe to call with null/undefined body (no-op).
 */
export function cleanupGridLabContext(body) {
  if (!body) return;
  if (body._gridLabUiTimer) clearTimeout(body._gridLabUiTimer);
  ...
  if (ctx?._gbVpLockUnsub) {
    ...
    ctx._gbVpLockUnsub = null;
  }
  if (ctx?._pollTimer) {
    clearInterval(ctx._pollTimer);
    ctx._pollTimer = null;
  }
  if (ctx?.gbLabSig) {
    try { ctx.gbLabSig.abort(); } catch (e) {}
  }
  if (ctx?.ro) {
    try { ctx.ro.disconnect(); } catch (e) {}
  }
  if (ctx?.lc) {
    try { ctx.lc.remove(); } catch (e) {}
  }
}

/**
 * Wire form input listeners on the backtest tab.
 * Pure-ish: only schedules sync via the injected scheduler and ratio helper.
 */
function bindGridLabBacktestForm(body, gbPrefs, scheduleGridLabSyncFn, applyRatioFn) {
  const bind = (id, reuseOnly) => {
    const el = body.querySelector(id);
    if (!el) return;
    el.addEventListener('input', () => scheduleGridLabSyncFn(body, gbPrefs, { reuseCandles: reuseOnly }));
    el.addEventListener('change', () => scheduleGridLabSyncFn(body, gbPrefs, { reuseCandles: reuseOnly }));
  };
  bind('#gbLevels', true);
  bind('#gbLow', true);
  bind('#gbHigh', true);
  bind('#gbLev', true);
  bind('#gbDep', true);
  bind('#gbGridMode', true);
  const ratioApply = body.querySelector('#gbRatioApply');
  if (ratioApply) ratioApply.onclick = () => applyRatioFn(body, gbPrefs);
  ...
  if (symEl) {
    symEl.addEventListener('change', () => {
      syncSymBounds();
      scheduleGridLabSyncFn(body, gbPrefs, { reuseCandles: false });
    });
  }
  const tfEl = body.querySelector('#gbTf');
  if (tfEl) {
    tfEl.addEventListener('change', () => scheduleGridLabSyncFn(body, gbPrefs, { reuseCandles: false }));
  }
}

/**
 * Public modal host. All call-site specific behavior is injected via deps:
 *   S                          - state for default-symbol resolution
 *   openFullscreenBySym        - click handler on selector rows
 *   scheduleGridLabSync        - debounced sync trigger
 *   applyGbRatioGrid           - ratio-based grid bounds applier
 *   gridLabBoundsUndo / Redo   - undo/redo of bounds edits
 */
export function renderGridLabModal(defSymOpt, deps = {}) {
  const {
    S,
    openFullscreenBySym,
    scheduleGridLabSync: scheduleFn,
    applyGbRatioGrid: applyRatioFn,
    gridLabBoundsUndo: undoFn,
    gridLabBoundsRedo: redoFn,
  } = deps;
  ...
  const gbKeydown = (e) => {
    ...
    if (!document.getElementById('gridLabModal')) {
    ...
    if (e.shiftKey) redoFn(body, gbPrefs);
    else undoFn(body, gbPrefs);
    e.preventDefault();
  };
  document.addEventListener('keydown', gbKeydown, true);

  const resolveDefSym = () =>
    defSymOpt || (S?.fsSym || S?.charts?.find((c) => c.sym)?.sym || S?.syms?.[0] || 'BTCUSDT');
  ...
  const setTab = (tab) => {
    ...
    if (tab === 'selector') {
      ...
      const fn = deps.fn || window.fn || ((v, d) => String(v));
      const fk = deps.fk || window.fk || ((v) => String(v));
      const openFnName = 'openFullscreenBySym';
      const openFullscreenBySymLocal = openFullscreenBySym || window[openFnName];
      const headerCols = ['Символ', 'GridScore', 'Ренж24', 'NATR5m', 'Δ24', 'Объём24', 'Сделки24'];
      body.innerHTML = `
      <div style="font-size:10px;color:var(--text3);margin-bottom:8px">Топ монет по пригодности к grid (ликвидность, ренж, mean-reversion, активность).</div>
      <div style="font-size:9px;color:var(--text3);line-height:1.4;margin:0 0 9px 0">
        GridScore = 24% ренж за 24ч + 20% NATR (5m) + 22% mean-reversion (меньше направленного тренда за 24ч) + 22% ликвидность (объём) + 12% активность (сделки). Чем выше score, тем обычно стабильнее и "рабочее" поведение для сетки.
        ...
        <div style="display:grid;grid-template-columns:110px repeat(6,1fr);gap:6px;font-size:9px">
          ${headerCols.map((h) => `<div style="color:var(--text3)">${h}</div>`).join('')}
          ${rows.map((r) => `
            <div style="font-weight:600;color:#fff;cursor:pointer" data-sym="${r.sym}" class="gb-selector-row">${r.sym.replace(/USDT$/, '')}</div>
            <div>${fn(r.score, 1)}</div>
            <div>${fn(r.range24, 2)}%</div>
            <div>${fn(r.natr, 2)}%</div>
            <div class="${r.ch24 >= 0 ? 'p' : 'n'}">${r.ch24 >= 0 ? '+' : ''}${fn(r.ch24, 2)}%</div>
      ...
      // Wire row clicks via event delegation (avoids inline JS that may not
      // work in strict CSP environments and is safer than string-eval).
      const handler = (ev) => {
        const row = ev.target.closest('.gb-selector-row');
        if (!row) return;
        const sym = row.dataset.sym;
        if (openFullscreenBySymLocal) openFullscreenBySymLocal(sym);
      };
      body.addEventListener('click', handler, { once: true });
      return;
    }
    const defSym = resolveDefSym();
    body.innerHTML = buildGridLabBacktestMarkup(gbPrefs, defSym);
    bindGridLabBacktestForm(body, gbPrefs, scheduleFn, applyRatioFn);
    scheduleFn(body, gbPrefs, { immediate: true });
  };
